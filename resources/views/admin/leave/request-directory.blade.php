...
<style>
    ...
    @media (max-width: 768px) {
        ...
        .admin-leave-mgmt-header {
            flex-direction: column !important;
            gap: 16px !important;
            align-items: stretch !important;
        }

        .admin-leave-mgmt-header .d-flex.gap-2.flex-wrap {
            width: 100%;
            justify-content: stretch;
        }

        .admin-leave-mgmt-header .kpi-stat {
            flex: 1;
            min-width: 0;
            padding: 12px 14px;
        }
        ...
        /* Filter bar - stack elements (unique class) */
        .admin-leave-mgmt-filter-bar {
        ...
        .admin-leave-mgmt-filter-bar > div:first-child {
            width: 100%;
            flex-direction: column !important;
            align-items: stretch !important;
            gap: 12px !important;
        ...
        .admin-leave-mgmt-filter-bar > div:first-child > div {
            width: 100%;
        }

        .admin-leave-mgmt-filter-bar .filter-select,
        .admin-leave-mgmt-filter-bar .filter-date-input {
            width: 100%;
            font-size: 0.8rem;
        }

        .admin-leave-mgmt-filter-bar .btn-export {
            width: 100%;
            justify-content: center;
        }

        /* Table - horizontal scroll (unique class) */
        .admin-leave-mgmt-table-wrap {
            overflow-x: auto;
            -webkit-overflow-scrolling: touch;
        }

        .admin-leave-mgmt-table-wrap .leave-table {
            min-width: 750px;
        }

        .admin-leave-mgmt-table-wrap .leave-table thead th {
            padding: 10px 12px;
            font-size: 0.68rem;
            white-space: nowrap;
        }

        .admin-leave-mgmt-table-wrap .leave-table td {
            padding: 12px 12px;
            font-size: 0.78rem;
        }
        ...
        /* Action buttons wrap */
        .admin-leave-mgmt-table-wrap .d-flex.gap-2 {
            flex-wrap: wrap;
            gap: 6px !important;
        }

        /* Pagination - stack (unique class) */
        .admin-leave-mgmt-pagination {
            flex-direction: column !important;
            gap: 12px !important;
            align-items: stretch !important;
        }

        .admin-leave-mgmt-pagination > span {
            text-align: center;
            font-size: 0.78rem;
        }

        .admin-leave-mgmt-pagination > div {
            width: 100%;
            display: flex;
            justify-content: center;
        }
...
@section('content')
    ...
    <div class="admin-leave-mgmt-header d-flex justify-content-between align-items-start mb-4 flex-wrap gap-3">
        <div>
    ...
    <form method="GET" action="{{ route('admin.employee_leave.index') }}"
          class="admin-leave-mgmt-filter-bar filter-bar d-flex align-items-end justify-content-between flex-wrap gap-3 mb-4">
        ...
        <a href="{{ route('admin.leave.export-csv', request()->query()) }}" class="btn-export">
            <i class="bi bi-download"></i> Export
        </a>
    </form>

    {{-- Table with unique class --}}
    <div class="admin-leave-mgmt-table-wrap leave-table-wrap mb-4">
    ...
    <div class="admin-leave-mgmt-pagination d-flex justify-content-between align-items-center mb-4">
...
<script>
    ...
    function openAdminLeaveRejectModal(leaveId) {
        document.getElementById('adminLeaveRejectForm').action = `${ADMIN_LEAVE_BASE_URL}/${leaveId}/reject`;
